...
- **高频**：出牌阶段通过 `set_attack_targets` action 预设多个攻击目标，`minion._pending_attack_targets = [target1, target2, ...]`。结算阶段按顺序消耗：第 1 次攻击取第 0 个，第 2 次取第 1 个……
- 预设目标耗尽后攻击英雄。
...
### 6.13 Card 位置追踪与卡实例事件
- **`Card._location`**：追踪卡牌当前所在区域，枚举值为 `"deck"`、`"hand"`、`"discard"`、`"board"`、`"exile"`、`"resolving"`。
...
- **执行方式**：扫描双方牌库中所有具有 `on_game_start` 属性的卡，随机排序后依次调用 `card.on_game_start(player, game, card)`。
- **代表卡牌**：血渍怀表（将自己置顶到牌库）。

### 6.16 跳过结算阶段（_skip_resolve_phase）
- **`Game._skip_resolve_phase`**：布尔标志，在 `resolve_phase()` 开头检查。若为 `True`，则跳过整轮攻击结算并清空标志。
- **代表卡牌**：钝锈指针（策略卡：本回合跳过结算阶段，获得一张衍生卡）。

### 6.17 双倍血契（_double_blood_gain）
- **`Player._double_blood_gain`**：动态布尔属性。为 `True` 时，献祭产生的鲜血数量自动翻倍（`temp_b *= 2`）。
...
| `adjacent_columns(col, board_size=5)` | 返回左右相邻列索引（不越界） |
| `find_unique_highest_attack(minions, key_fn)` | 找唯一最高攻异象；并列返回 None |
| `freeze_minion(minion, layers=1)` | 为异象添加冰冻关键词（统一接口） |
| `freeze_enemies_in_columns(game, player, columns, layers=1)` | 冰冻指定列的所有敌方异象 |
...
- [x] **Card 位置追踪**：`Card._location` + `move_to()` 统一追踪卡牌所在区域。
- [x] **卡实例级事件监听**：`Card.on()` / `off_all()` 支持卡实例注册 EventBus 监听器。
...
- 当前人工实现进度（详见 `COMPLEX_CARDS_INDEX.md`）：
  ...
  - 血契包：**策略卡全部完成**（~30 张）。异象卡 ~5 张已实现（保卫者、巫毒娃娃、天籁人偶、溴化银、亡灵），~35 张异象卡 TODO（含管窥等开发机制卡）。
...
### 13.7 未实现的关键词
- 护盾、嘲讽、圣盾、战吼、沉默、吸血、超杀、冻结（区别于冰冻）、无法攻击、魔免、复生、剧毒、激励、发现（区别于开发）
...
| `card_pools/discrete.py` | 离散卡包 182 张卡牌定义（166 张已实现，12 张策略卡待实现） |
...
> 以下术语在 Tards 规则中有**精确定义**，与日常语义或其他 TCG（炉石/万智/YGO）可能完全不同。**禁止望文生义**。

| 用语 | 正确定义 | 常见错误 |
|------|---------|---------|
| **相邻** | 上下左右 **4 格**（曼哈顿距离 1） | 误认为包含对角线（8 格） |
| **周围** | 包含对角线的 **8 格**（切比雪夫距离 1） | 与"相邻"混淆 |
| **弃置** | 通过 `discard_card()` 或手牌满磨牌触发 `EVENT_DISCARDED` | 误认为正常打出策略卡进入弃牌堆也算弃置 |
| **打出 / 进入弃牌堆** | 正常消耗卡牌进入弃牌堆，**不触发**弃置相关效果 | 与"弃置"混淆 |
| **造成伤害** | 使用 `health_change(-delta)` 或 `take_damage()`，**受坚韧/伤害替换影响**，触发血契 2 级 | 与"流失生命"混淆 |
| **流失生命值** | 使用 `lose_hp(delta)`，**不受坚韧**，**不触发**血契 2 级和伤害替换 | 与"造成伤害"混淆 |
| **抽取** | `hidden_keywords={"抽取": callback}`，卡牌**被从牌库抽入手牌时**触发 | 与"抽牌"（动作）混淆；误认为直接 append 到手牌也能触发 |
| **抽牌** | `draw_card()` 动作，将卡从牌库移至手牌 | 与"抽取"混淆 |
| **获得槽位** | 增加上限（`t_point_max += 1`），**不自动获得**可用资源点 | 误认为同时获得资源点 |
| **获得资源点** | 增加当前可用资源（`t_point_change(1)`） | 与"获得槽位"混淆 |
| **唯一最高** | 必须有且仅有 **1 个**对象满足条件；并列时不触发 | 多个并列时仍触发 |
| **爆掉** | 直接销毁，**不进任何区域**，不触发任何事件 | 与"弃置"或"磨牌"混淆 |
| **磨牌** | 手牌满时抽牌，卡进入**弃牌堆**，触发 `EVENT_DISCARDED` | 与"爆掉"混淆 |
| **如可能** | 没有资源/目标时**直接跳过**，不报错、不失败 | 资源不足时报错或视为失败 |
| **如可能，消耗 X** | 先检查资源是否足够，足够才消耗并执行后续；不足则整句跳过 | 部分执行（消耗了但后续失败） |
| **视野** | 允许攻击**列范围内**的敌方异象；**不能看破潜水/潜行** | 误认为可以选中结算阶段的潜水/潜行 |
| **潜水 / 潜行** | **行动阶段**可见且可选中（策略可以指向）；**结算阶段**攻击自动落空 | 误认为行动阶段不可选中 |
| **移动** | 内部效果回调（如 `auto_effects.move_minion`），**不是玩家行动类型** | 误认为玩家可以主动"移动"结束回合 |
| **高频** | 每回合攻击多次，每次先攻 -1；**不自动要求选择目标** | 误认为高频自带视野 |
| **恐惧** | 清除所有通用词条并加紫色边框；**不是"无法攻击"** | 与"眩晕/冰冻"混淆 |

### 一句话速查

- **相邻 ≠ 周围**：相邻 4 格，周围 8 格。
- **弃置 ≠ 打出进弃牌堆**：只有前者触发弃置事件。
- **造成伤害 ≠ 流失生命**：前者吃坚韧，后者不吃。
- **抽取 ≠ 抽牌**：抽取是被抽入时触发的关键词机制。
- **获得槽位 ≠ 获得资源点**：加上限不加当前值。
- **爆掉 ≠ 磨牌**：爆掉是直接消失，磨牌是进弃牌堆。
- **唯一最高**：并列 = 不唯一 = 不触发。
- **如可能**：缺资源/目标 = 无事发生。
...
## 二十一、本轮对话（2026-05-06）核心交付

### 21.1 词条改动
1. **「异放n」（原「回响n」）**：
   - 异象部署时：创建 2T/1/1 复制体，`echo_level - 1`。
   - 策略/矿物打出时：`copy.copy` 后 `echo_level - 1`。
   - 所有策略卡原「回响」统一改为「异放」。
2. **「回响」新定义**：
   - 异象 keywords 含「回响」时，费用强制 2T，面板 1/1，其余词条不变。
   - 由 `player.py` 部署逻辑和 `_get_play_cost` 统一处理。

### 21.2 架构/引擎改动
1. **`Player.draw_card` 新增 `_acquired_by_draw` 标记**：
   - 抽到的卡设置 `card._acquired_by_draw = True`，供「复兴」等效果区分「抽牌获得」与「非抽牌获得」。
2. **`Player._cost_modifiers` 玩家级费用修正**：
   - 已存在，被 C418 等卡直接利用（修改 `card.cost.t`）。
3. **`__all__` 导出列表维护**：
   - `discrete_effects.py` 所有策略/特殊函数必须加入 `__all__`，否则 `discrete.py` 的 `from .discrete_effects import *` 无法导入。

### 21.3 离散卡包人工效果编译进度
- **已完成（约 166 张）**：雪块、发射器、哭泣的黑曜石、高炉、青金石、铁镐、多重射击、火矢、横扫之刃、饵钓、忠诚、击退、深海探索者、保护、效率、经验修补、砍伐！、掘进！、铁砧、冶炼、脆弱同盟、一日八秋、整装上阵、垂直竖井、鱼骨挖掘、迷失、蛀蚀、遗弃、复兴、高山、恶地、雨林、雪原、火药、TNT、复制技术、轰击、制导技术、珍珠塔、矢量炮、实体挤压、C418、红石粉、禁人书 等。
- **剩余 TODO（12 张策略卡）**：
  1. 金西瓜片(铁) 1G — 使1个异象获得：回合开始：获得+1/1。
  2. 雷暴(铁) 7T — 随机消灭2个敌方异象。
  3. 阴雨(铁) 2T — 对1个非高地异象造成2点伤害，将其攻击力设为0直到回合结束。
  4. 暗夜(铁) 4T — 开发1张离散迅捷异象。然后若你拥有不小于4个T点，对所有敌方目标造成1点伤害。
  5. 破袭(铁) 5T — 使1个友方异象与1个攻击力最低的敌方异象对战。若将其消灭，获得+1HP并重复此流程。
  6. 火灾(铁) 1D — 你和对手轮流抽牌至手牌数量为7张，下回合开始时弃掉本次抽到的牌。
  7. 遗迹机关(铁) 6T — 将2张「绊线钩」加入战场。然后若场上有不多于3个「绊线钩」，对所有目标造成1点伤害。
  8. 临界点(铁) 1G — 开发1张非生命异象，使其具有「受到伤害前，每有1个友方绊线钩与之同行或同列，对1个随机敌方目标造成1次1点伤害。」
  9. 药水箭(铁) 4T — 使1个异象获得+1/2，抽1张牌。若指向非生命异象，再使所有友方非生命异象获得+2HP。
  10. 熔岩(铁) 6T — 抉择：对所有后排异象造成4点伤害 或 对对手造成6点伤害。
  11. 充能铁轨(铁) 1G 异放2 — 移动1个陆地异象。若是友方异象，抽1张牌。
  12. 调试棒(铁) 4T — 抽1张策略，将其展示并获得-1T花费和「抽1张策略，将其展示并获得-1T花费」。

### 21.4 关键实现模式（本轮新增范例）
- **「复兴」**（持续监听）：利用 `on("phase_end", ...)` + `_acquired_by_draw` 标记，实现「非抽牌获得手牌时触发」。
- **「C418」**（临时费用修改）：直接修改 `card.cost.t` + `game.history.unlisten()` 自注销。
- **「垂直竖井」**（击鼓传花）：`effect_fn` 接收第5参数 `card`，在复制卡上递增 `_chuizhushujing_draw` 和 `cost.t`。
- **「实体挤压」**（延迟触发）：给 `minion.on_phase_start` 赋值一次性回调，触发后设为 `None` 自注销。
- **「禁人书」**（AOE 关键词）：`gain_keyword(..., permanent=False)` 实现临时眩晕。
- **「冶炼」**（全局组合搜索）：`itertools.combinations` 筛选费用和匹配的非生命异象，直接 `place_minion` 不触发部署效果。
- **「鱼骨挖掘」**（开发+直接放置）：`game.request_choice` 3选1 + `Minion` 手动创建 + `board.place_minion`。

### 21.5 已知遗留问题
- **Token 费用漏洞**：`summon_token()` 默认 `Cost()`（0费），被 `return_minion_to_hand` 返回手牌后可免费再打出。用户要求以后新 token 默认 3T，现有 token 尚未批量处理。
- **Pylance 静态分析**：`from .discrete_effects import *` 导致部分函数名报「未定义」，不影响运行时，但建议显式导入或保持 `__all__` 完整。
- **「火灾」机制复杂**：需要记录「本次抽到的牌」并在下回合开始时弃置，涉及 `Player` 状态标志。
- **「临界点」触发器复杂**：需要给异象动态注入「受到伤害前」回调，并关联场上「绊线钩」数量。

---

*文档版本：2026-05-06*  
*涵盖规则书 v1.0 + 离散卡包（166/182 实现）+ 冥刻卡包 v1.0 + 血契卡包 + 程序实现全状态 + 统一指向模块 + 堆栈响应机制 + 轻量取消机制 + 人工效果协作规范 + 献祭重构 + 测试卡组模式 + Card 位置追踪 + 抽取机制 + 对局开始时效果 + 回响/异放词条改动*
