...
register_card(
    ...
    attack=4,
    health=4,
    # 效果描述：无法选中HP不大于2的单位。回合开始：对一个本列单位造成2点伤害。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=3,
    keywords={"视野": 2, "高频": 3},
    # 效果描述：受到其伤害的目标此前每被本单位指向一次，受到的伤害+1.
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    tags=['两栖'],
    # 效果描述：部署：消灭场上攻击力最低的单位，获得被消灭单位的攻击力与防御力。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：对HP不小于本单位的目标，伤害+1。亡语：对全体敌方目标造成1点伤害。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：友方单位攻击后，若攻击目标 HP不小于2，令攻击目标失去1点HP。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    tags=['两栖'],
    evolve_to="西瓜",
    # 效果描述：回合结束：场上单位更少的一方抽一张牌。亡语：将一张“西瓜”加入原位。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=8,
    keywords={"坚韧": 1, "先攻": -1},
    # 效果描述：部署：对所有单位造成1点伤害。回合开始时，随机眩晕一个敌方单位。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    tags=['两栖'],
    # 效果描述：友方单位被消灭后，加入其位置并攻击。友方单位部署时，本单位返回手牌。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：（雕像激活后将在回合结束时移除 保留增益） 激活时：所有友方昆虫单位具有亡语；对对手造成1点伤害。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：（只有“上下匹配”的雕像可以在一回合内拼装 否则需要两回合） 融合：激活座首，所有友方昆虫单位进入战场时+1/1。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：激活时：每回合首个友方B=0单位入场时献祭等级+1。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    keywords={"成长": 2},
    evolve_to="成鸟",
    # 效果描述：部署：指向一个友方飞禽单位，使其获得+3防御力和“友方幼鸟无法选中”。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    attack=1,
    health=1,
    # 效果描述：组队 每有一个其它单位被献祭，+1/1，丰饶等级+1。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    keywords={"两栖": True},
    tags=['两栖'],
    # 效果描述：组队 回合开始：重置一个单位的攻击力与防御力，你获得+1HP。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：所有花费≤5的非飞禽单位部署时具有休眠I。 部署：使敌方花费不大于4的单位返回手牌。
    targets_fn=target_friendly_positions,
...
register_card(
    ...
    tags=['水生'],
    # 效果描述：平地均算作是水路。 部署：使全体友方非两栖单位获得“水生”。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    evolve_to="老鹿",
    # 效果描述：你的手牌花费-1T。成长时，若场上有B=0单位，改为重置计时。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=4,
    keywords={"坚韧": 1},
    # 效果描述：场上有不少于3个友方单位时，花费-2B。部署：开发一张卡组中的牌。
    targets_fn=target_friendly_positions,
...
register_card(
    ...
    health=3,
    keywords={"协同": True},
    # 效果描述：敌方目标被指向后，也算作是被本单位指向。 回合开始：使所有被本单位指向的目标失去1点HP。 部署：指向一个目标。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=3,
    keywords={"防空": True},
    # 效果描述：若这是你本回合部署的最后一个单位，获得迅捷。 你部署献祭点数大于0的单位时，对对手造成1点伤害。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：回合结束：使同列敌方前排单位移动至后排，后排单位返回对方手牌。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    tags=['水生'],
    # 效果描述：对空袭与昆虫单位伤害翻倍。受到本单位伤害的单位本回合改为在回合结束时 攻击。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=5,
    keywords={"视野": 1, "先攻": 1},
    # 效果描述：攻击时，改为与目标对战。消灭一个单位后，获得等同于其攻击力的HP。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：将其消灭的单位的回响加入手牌。你打出或弃掉回响时，本单位返回手牌。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    tags=['两栖'],
    # 效果描述：回合结束：向相邻方向移动一格。 将受到其伤害的单位的回响加入手牌，回合开始时弃掉。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=5,
    keywords={"协同": True, "坚韧": 1},
    # 效果描述：相邻单位受到伤害时，改为由本单位承受。 回合结束：若本单位本回合未受到伤害，对对手造成2点伤害。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    # 效果描述：必须是本回合双方部署的第2个单位。 亡语：若消灭过单位，随机消灭一个敌方单位。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    tags=['水生'],
    evolve_to="水螅群",
    # 效果描述：亡语：若是由于单位效果被消灭，改为在回合结束时成长。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=4,
    keywords={"协同": True},
    # 效果描述：对方所有单位花费+2T。对方单位部署前，本单位返回手牌。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=2,
    keywords={"亡语": True},
    # 效果描述：部署：指向一个单位。亡语：使伤害来源与一个随机敌方单位对战。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    health=6,
    keywords={"坚韧": 1},
    # 效果描述：场上有昆虫类单位时，HP无法降至1以下。 回合开始：将1张“兵蚁”加入战场。
    targets_fn=target_friendly_positions,
    special_fn=None,  # TODO: 实现部署/回合效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：将一只“松鼠”加入战场，本回合所有B=0友方单位无法选中。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：对你造成2点伤害，然后对一个单位造成4点伤害。抽一张牌。
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=3,
    # 效果描述：选择并弃一张单位，若其献祭等级与丰饶等级之积不小于2，将一张“骨王之 赏”加入手牌；否则将一张“骨王之惠”加入手牌。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=2,
    # 效果描述：选择并弃一张单位，使你手牌中另一张同名单位攻防翻倍且花费+1T。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：随机将一张手牌洗入卡组。抉择：抽2张单位或获得4T。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=2,
    # 效果描述：使一个单位获得亡语：随机对一个友方单位造成2点伤害，使其获得此亡语。
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：使一个单位获得：在受到致命伤害前，+0/1。若因此存活，+1/1。
    targets_fn=target_any_minion,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：弃掉一个花费不大于7T的单位，在3回合后的抽牌阶段将其加入战场。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=2,
    # 效果描述：使手牌中一个单位获得 +2/3 和亡语：对方抽一张牌。
    targets_fn=target_none,
    effect_fn=lambda p, t, g, extras=None: (p.draw_card(1, game=g) or True),
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：使一个友方单位获得 +0/4 并冰冻2回合，期间其拥有坚韧I。
    targets_fn=target_friendly_minions,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=3,
    # 效果描述：本回合你每部署1个单位，花费-1T。你获得1个T槽，对方失去1个T槽。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：对所有陆地单位造成3点伤害，然后使所有陆地单位+1/2。你失去1个T槽。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=2,
    # 效果描述：若水路有敌方单位，对所有敌方单位造成1点伤害，若有单位被消灭，重复此 流程。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=2,
    # 效果描述：眩晕一个敌方单位。若本回合对方先手，结束双方出牌阶段。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=2,
    # 效果描述：将2张“松鼠”加入手牌。本回合，你每献祭一个单位，抽一张牌。
    targets_fn=target_none,
    effect_fn=lambda p, t, g, extras=None: (p.draw_card(1, game=g) or True),
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：使一个单位获得亡语：将其-1/1的复制加入你的手牌。
    targets_fn=target_any_minion,
    effect_fn=lambda p, t, g, extras=None: (setattr(t, "base_keywords", dict(getattr(t, "base_keywords", {}))) or t.base_keywords.update({"亡语": True}) or t.recalculate() or True) if hasattr(t, "recalculate") else False,
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：选择并弃1张牌，将一个单位返回其所有者牌堆顶。眩晕周围单位。
    targets_fn=target_any_minion,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：选择并弃1张单位，将其花费为5T的复制加入对方手牌。对方部署其它单位时，将此单位加入友方区域。
    targets_fn=target_none,
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：对方部署单位前，将其移除，在下回合开始后将其加入原位。
    targets_fn=target_none,
    condition_fn=None,  # TODO: 实现触发条件
    effect_fn=None,  # TODO: 实现效果
)
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：对方打出下一张牌时，若是单位，对对手造成等同于其花费的伤害。
...
register_card(
    ...
    rarity=Rarity.IRON,
    immersion_level=1,
    # 效果描述：敌方单位对你造成伤害前，使其先获得-2攻击力。若具有迅捷，将其消灭。
    targets_fn=target_none,
    condition_fn=None,  # TODO: 实现触发条件
    effect_fn=None,  # TODO: 实现效果
)
