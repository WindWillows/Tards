...
每次完成一张（或一批）卡牌后，**必须**使用以下格式向用户汇报。不可省略"定位与强度评测"部分。
...
| **关键词** | `<关键词列表，无则写"无">` |
| **效果简述** | `<一句话描述核心机制>` |
| **实现文件** | `card_pools/<包名>_effects.py` 中的 `<函数名>` |
...
|------|------|
| **节奏定位** | `<前期/中期/后期/全能>` |
| **核心作用** | `<场面争夺/资源引擎/干扰控制/终结手段/其他>` |
| **配合潜力** | `<高/中/低，列举 1-2 个关键配合>` |
| **独立强度** | `<亏模/标准/超模，依据费用—强度模型 §4 判断>` |
| **设计风险** | `<如：快攻组件需注意上限控制、高费卡需确保灵活性补偿、与某张已有卡可能产生极端 combo 等>` |

> 若效果实现与文本描述存在**任何歧义或简化处理**，必须在汇报中明确标出，并说明理由。

---

## AskUserQuestion 使用规范

实现任何卡牌效果前，**若遇到以下情况，必须使用 `AskUserQuestion` 工具向用户确认**，禁止擅自推断：

1. **效果文本存在歧义** — 同一句话可以有多种合理解释
2. **触发时机不明确** — "回合开始时"具体指结算阶段开始还是完整回合开始
3. **目标选择范围模糊** — "一个敌方异象"是否包含潜水/潜行单位
4. **数值边界情况** — 效果在极端情况（如空场、满场、0 费用）下的行为
5. **与其他机制的交互** — 该效果与现有卡牌（尤其是已实现的同包卡牌）可能产生非直观交互
6. **关键词定义冲突** — 卡牌描述使用了未定义或新定义的关键词

**提问原则**：

- 每次提问 **1-4 个**相关问题，每组问题提供 **2-4 个**选项
- 选项必须包含**明确的实现差异说明**，而非模糊的"是/否"
- 若用户此前已明确答复过同类问题，可直接引用，无需重复提问
- **必须**等待用户答复后再继续实现，不可边问边写
