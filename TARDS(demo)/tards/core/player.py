...
class Player:
    def __init__(self, side: int, name: str, diver: str, card_deck: List[Card], original_deck_defs: Optional[List[Any]] = None):
        ...
        self.b_point = 0
        self.s_point = 0
        # T槽自然增长上限修正（如“污煤”使其降低）
        self._natural_t_max_cap_modifier = 0
    ...
    def play_card(self, serial: int, target: Any, game: "Game", bluff: bool = False, extra_targets: Optional[List[Any]] = None) -> bool:
        ...
        if isinstance(card, MinionCard):
            ...
            def deploy_fn():
                ...
                # 记录本次可用于支付鲜血费用的非献祭B点数量，
                # 此时 b_point 已被 cost.pay 扣除，b_point + cost.b 等于扣费前数量。
                card._blood_paid_from_b_point = min(self.b_point + cost.b, cost.b)
                ...
                if effect:
                    ...
                    self.c_point_change(cost.c)
                    cost.rollback(self)
                    if hasattr(card, '_blood_paid_from_b_point'):
                        delattr(card, '_blood_paid_from_b_point')
