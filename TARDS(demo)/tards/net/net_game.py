...
from ..data.card_db import DEFAULT_REGISTRY, Pack
from ..core.targeting import get_attack_target_candidates
...
class NetworkDuel:
    ...
    def __init__(
        ...
        # 本地玩家行动同步（使用队列，避免 rapid auto-fill 覆盖）
        self._local_action_queue: queue.Queue[Dict[str, Any]] = queue.Queue()
        ...
        # 对手决策状态可视化
        self._remote_decision_state: Optional[str] = None
    ...
    def connect(self) -> bool:
        ...
        try:
            if self.is_host:
                if self.use_ngrok:
                    ...
                    if not url:
                        return False
                self.conn = start_host(self.port, self._shutdown_event)
            else:
                if not self.host_ip:
        ...
        if not hello:
            print("[Net] 等待 HELLO 超时或对方断开")
            self._cleanup_connection()
            return False
        # 校验 HELLO 消息字段
        name = hello.get("name")
        if not isinstance(name, str) or not name.strip():
            self.connect_error = "对手 HELLO 消息缺少合法名称"
            print(f"[Net] {self.connect_error}")
            self._cleanup_connection()
            return False
        self.remote_name = name

        remote_deck = hello.get("deck")
        if not isinstance(remote_deck, list) or not all(isinstance(x, str) for x in remote_deck):
            self.connect_error = "对手卡组格式非法"
            print(f"[Net] {self.connect_error}")
            self._cleanup_connection()
            return False
        self.remote_deck_list = remote_deck

        raw_imm = hello.get("immersion_points", {})
        if not isinstance(raw_imm, dict) or not all(
            isinstance(k, str) and isinstance(v, int) for k, v in raw_imm.items()
        ):
            self.connect_error = "对手沉浸点格式非法"
            print(f"[Net] {self.connect_error}")
            self._cleanup_connection()
            return False
        self.remote_immersion_points = dict(raw_imm)
        ...
        if self.is_host:
            ...
            if not start_msg:
                print("[Net] 等待 START 超时或对方断开")
                self._cleanup_connection()
                return False
            first_name = start_msg.get("first_player_name")
            seed = start_msg.get("seed", 0)
            if not isinstance(first_name, str) or not isinstance(seed, int):
                self.connect_error = "START 消息字段非法"
                print(f"[Net] {self.connect_error}")
                self._cleanup_connection()
                return False
            self.first_player_name = first_name
            self.seed = seed
    ...
    def run_game(self, opponent: Player, action_provider: Optional[Callable] = None, logger=None):
        """在后台线程中启动游戏。"""
        print(f"[Net] run_game 开始: is_host={self.is_host}", file=sys.stderr)
        ...
        print(f"[Net] 创建 Game 对象...", file=sys.stderr)
        self.game = Game(
            first,
            second,
        ...
        print(f"[Net] 启动同步哈希监听...", file=sys.stderr)
        ...
        print(f"[Net] 调用 game.start_game()", file=sys.stderr)
        self.game.start_game()
        if self.game.game_over:
            # 先通知远端游戏结束，再触发本地 UI，避免对方误报“连接断开”
            self.notify_gameover(self.game.winner.name if self.game.winner else None)
        ...
        def listener():
            while self.conn and self.conn.is_running() and not getattr(self.game, "game_over", False):
                ...
                if mtype == "SYNC_HASH":
                    ...
                    self._remote_decision_state = msg.get("state")
    ...
    def _make_action_provider(self):
        def provider(game, active, opponent):
            if active == self.local_player:
                # 进入本地回合，清除对手决策状态
                self._remote_decision_state = None
                # 通知 GUI
                if self.local_turn_callback:
                    self.local_turn_callback()
                # 等待本地玩家提交 action；使用队列避免 rapid auto-fill 覆盖
                ...
                self._local_turn_event.clear()
                try:
                    action = self._local_action_queue.get(block=False)
                except queue.Empty:
                    return {"type": "brake"}
                # 若队列中仍有 action，保持事件 set，让下次 provider 立即返回
                if self._local_action_queue.empty():
                ...
                while True:
                    ...
                    if msg:
                        ...
                        if mtype == "DISCOVER":
                            ...
                            if not isinstance(chosen_name, str) or chosen_name not in names:
            ...
            if request.deciding_player == self.local_player:
                ...
                while True:
                    ...
                    if msg:
                        ...
                        if mtype == "TARGETING":
                            try:
                                from .net_protocol import _deserialize_target
    ...
    def submit_local_action(self, action: Dict[str, Any]):
        ...
        高频输入时，主行动（出牌/拍铃/拉闸/兑换等）只保留最新一个，避免队列中
        堆积 stale 主行动导致后续回合误执行；目标设置类行动（攻击/效果预设）
        可以批量入队，以支持 auto-fill 为多个异象同时设置目标。
        """
        atype = action.get("type")
        is_target_action = atype in ("set_attack_targets", "set_effect_target")

        if not is_target_action:
            # 主行动：丢弃队列中已有的旧主行动，避免 stale action 被后续消费
            kept = []
            while True:
                try:
                    old = self._local_action_queue.get_nowait()
                    if old.get("type") in ("set_attack_targets", "set_effect_target"):
                        kept.append(old)
                except queue.Empty:
                    break
            for old in kept:
                self._local_action_queue.put(old)

        self._local_action_queue.put(action)
        self._local_action_event.set()

    def is_local_turn(self) -> bool:
        return self._local_turn_event.is_set()

    def _send_state(self, state_type: str) -> None:
        """向远端发送当前本地玩家所处的决策状态。"""
        if self.conn and self.conn.is_running():
            try:
                self.conn.send(msg_state(state_type))
            except Exception as e:
                print(f"[Net] 发送状态消息失败: {e}")
        ...
        def provider(game, players):
            ...
            while True:
                ...
                if msg:
                    mtype = msg.get("type")
                    if mtype == "MULLIGAN":
                        raw_indices = msg.get("indices", [])
                        if isinstance(raw_indices, list) and all(isinstance(i, int) for i in raw_indices):
                            remote_indices = raw_indices
                        else:
                            print(f"[Net] 非法 Mulligan 索引 {raw_indices}")
                            game.game_over = True
                            remote_indices = []
                        break
                    if mtype in ("GAMEOVER", "DISCONNECT"):
    ...
    def close(self):
        # 标记已通知，避免本地主动关闭后 _recv_loop 再次触发 disconnect 回调
        with self._disconnect_lock:
            self._disconnect_notified = True
        # 取消仍在进行的连接等待
        ...
        self._mulligan_event.set()
        if self.conn:
            # 正常对局结束时不发送 DISCONNECT，避免对方误报“连接断开”
            if not (self.game and self.game.game_over):
                try:
