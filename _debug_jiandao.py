...
# Monkey-patch t_point_change to trace
orig_tpc = Player.t_point_change
def traced_tpc(self, delta):
    old = self.t_point
    orig_tpc(self, delta)
    new = self.t_point
    print(f'  [TRACE] {self.name}.t_point_change({delta}): {old} -> {new}')
Player.t_point_change = traced_tpc
...
defs = [d for d in DEFAULT_REGISTRY.all_cards() if d.name == '剪刀' and d.card_type == CardType.CONSPIRACY]
jiandao = defs[0].to_game_card(p1)

p1.card_hand = [jiandao, MinionCard(name='TestMinion', owner=p1, cost=Cost(t=1), targets=target_friendly_positions, attack=1, health=1)]
p2.card_hand = [MinionCard(name='EnemyMinion', owner=p2, cost=Cost(t=1), targets=target_friendly_positions, attack=1, health=1)]
...
def actor(game, active, opponent):
    ...
    if game.current_turn == 1 and active.name == 'P1' and state['step'] == 1:
        state['step'] = 2
        return {'type': 'play', 'serial': 6, 'target': (3, 0)}
    if game.current_turn == 1 and active.name == 'P1' and state['step'] == 2:
        state['step'] = 3
        return {'type': 'brake'}
    if game.current_turn == 2 and active.name == 'P2' and state['step'] == 3:
        state['step'] = 4
        print(f'  [TRACE] Before P2 play: T={active.t_point}')
        return {'type': 'play', 'serial': 1, 'target': (0, 0)}
    if game.current_turn == 2 and active.name == 'P2' and state['step'] == 4:
        state['step'] = 5
        print(f'  [TRACE] After P2 play/conspiracy: T={active.t_point}')
        return {'type': 'brake'}
    return {'type': 'brake'}
